<template>
  <div v-for="(product,index) in products" :key="index" class="w-full md:w-1/3 xl:w-1/4 p-6 flex flex-col">
    <div>
      <img @click="handleClick(product)" class="hover:grow hover:shadow-lg" :src="product.image">
      <div class="pt-3 flex items-center justify-between">
        <p class="text-xl font-bold">{{product.name}}</p>
        <button @click="handleAddToCart(product)" class="p-2 rounded-full bg-blue-600 text-white mx-5 -mb-4 hover:bg-blue-500 focus:outline-none focus:bg-blue-500">
